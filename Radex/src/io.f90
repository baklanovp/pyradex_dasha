...
contains
   ...
   subroutine getinputs
      ...
      read(*,20) molfile
      if ((molfile(1:1).ne.'/').and.(molfile(1:1).ne.'.'))   molfile = trim(radat)//trim(molfile)
...
44       write(*,22) 'Type of partner',ipart,' ? '
         ...
         if (id.eq.0) then
            print *, 'Unknown species. Choose from: H2 p-H2 o-H2 e H He H+'
            goto 44
         endif
         write(13,*) partner
...
!     Add ortho and para H2 densities if applicable
      if ((density(2).gt.0.0).or.(density(3).gt.0.0)) density(1)=density(2)+density(3)
...
!     Line flux in K*cm/s and in erg/s/cm^2
