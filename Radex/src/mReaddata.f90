...
subroutine readdata
   ...
   do iline=1,nline
      read(11,*) dummy,iupp(iline),ilow(iline),aeinst(iline),spfreq(iline),eup(iline)
...
102 format(i1,a)
   do ipart=1,npart
      ...
      if (is_debug) write(*,*) 'ready to read ',ncoll,' rates for ',ntemp,' temperatures for partner ',ipart

      do icoll=1,ncoll
         read(11,*) dummy,lcu(icoll),lcl(icoll),(coll(ipart,icoll,itemp),itemp=1,ntemp)
         ...
         if(tkin.gt.temp(1)) then
            if(tkin.lt.temp(ntemp)) then
               ...
               do icoll=1,ncoll
                  ...
                  if(colld(ipart,iup,ilo).lt.0.0) colld(ipart,iup,ilo)=coll(ipart,icoll,nint)
               enddo
            else
               ! C===  Tkin too high :
               if (tkin.ne.temp(ntemp)) then
                  print*,' Warning : Tkin higher than temperatures '//'for which rates are present.'
               endif
               do icoll=1,ncoll
                  iup=lcu(icoll)
                  ilo=lcl(icoll)
            ...
            if (tkin.ne.temp(1)) then
               print*,' Warning : Tkin lower than temperatures '//'for which rates are present.'
            endif
            do icoll=1,ncoll
               iup=lcu(icoll)
               ilo=lcl(icoll)
   ...
   do ipart=1,maxpart
      ...
      do jpart=1,maxpart
         if ((id(jpart).eq.ipart).and.(density(ipart).gt.0.d0)) then
            ...
            do iup=1,nlev
               do ilo=1,nlev
                  crate(iup,ilo) = crate(iup,ilo) + density(ipart)*colld(jpart,iup,ilo)
               enddo
            enddo
         endif
      enddo
      ...
      do ilo = 1,nlev
         ...
         if(ediff.gt.0.0d0) then
            if((fk*ediff/tkin).ge.160.0d0) then
               ...
               crate(ilo,iup) = gstat(iup)/gstat(ilo)*dexp(-fk*ediff/tkin)*crate(iup,ilo)
            endif
         endif
      enddo
   ...
   !     if (is_debug) print*,'ctot=',(ctot(ilev),ilev=1,nlev)
