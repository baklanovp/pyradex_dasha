...
contains
   ...
   subroutine backrad
      ...
      !      ! highest allowed by f90 (fdvt 28apr06)
      real*8 aa,bb,fout
...
!        xlmin = dlog10(xnumin)
!        xlmax = dlog10(xnumax)
!        step = (xlmax-xlmin)/float(nstep)

!        do istep = 1,nstep+1
!           xl  = 10.d0**(xlmin + float(istep-1)*step)
!           xln = dlog(xl)

!           if(xl.lt.xnubr(1).or.xl.gt.xnubr(nrad)) then
!            print*, ' Warning:  the line list requires extrapolation of t
!      ;he continuum radiation '
!            write(8,834)
!  834       format(' WARNING: the continuous background requires extrapol
!      ;ation beyond the specified frequencies',/,' for some lines in the
!      ;line list')
!           endif
...
!              cbi = 0.0d0
!           endif
!           toti = 1.0d-14*dexp(fout) + cbi
!           trad = bb/dlog(aa/toti + 1.0d0)
!        enddo
...
!     yy,xla,ylg: to calculate stellar radiation field
      ...
      do iline = 1,nline
         ...
         elseif(xnu(iline).le.109678.76d0) then
            ...
            write(*,202) xnu(iline)
            202         FORMAT(' ** XNU = ',1PE13.6,' IS OUTSIDE THE RANGE OF THE FITTING; FUNCTION AND BEYOND LYMAN LIMIT')
            cbi=0.0d0
            cmi=0.0d0
