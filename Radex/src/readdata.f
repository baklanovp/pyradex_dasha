...
c     Quite a big IF:
c     if ((npart.gt.1).and.
c    $   (density(1).gt.eps).and.(density(2).lt.eps)
c    $     .and.(density(3).lt.eps)) then
c        opr        = min(3.d0,9.0*dexp(-170.6/tkin))
c        density(2) = density(1)/(opr+1.d0)
c        density(3) = density(1)/(1.d0+1.d0/opr)
c        print*,'*** Warning: Assuming thermal o/p ratio for H2 of ',opr
c     endif
